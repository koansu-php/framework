<?php
/**
 *  * Created by mtils on 23.10.2022 at 20:25.
 **/
 ...
 * Class Command
 *
 * A command is the console counterpart to a route. In Koansu you define your
 ...
 * as with routes.
 *
 * Like in Koansu\Core\Url: Properties are for read access, methods for
 * write access.
 *
...
class Command implements Arrayable, JsonSerializable
{
     ...
     */
    public function __construct(string $pattern, string $description='', RouteCollector $routeCollector=null)
    {
     ...
     */
    public function argument($signature, string $description='') : Command
    {
        // Many arguments at once?
        if (is_array($signature)) {
            array_map([$this, 'argument'], $signature);
            return $this;
        }
     ...
     * @noinspection PhpMissingParamTypeInspection
     */
    public function option($signature, string $description='', string $shortcut='') : Command
    {
        // Many arguments at once?
        if (is_array($signature)) {
            array_map([$this, 'option'], $signature);
            return $this;
        }
     ...
     * @return $this
     */
    public function setRoute(string $pattern, Route $route) : Command
    {
     ...
     * @noinspection PhpMissingParamTypeInspection
     */
    public function on($method, string $pattern) : Route
    {
     ...
     * @return Argument
     */
    protected function parseArgumentSignature(string $signature) : Argument
    {
        $isOptional = false;
        $type = null;

        // If starts or ends with ? its optional
        if (Str::stringStartsWith($signature, '?') || Str::stringEndsWith($signature,'?')) {
            $isOptional = true;
        ...
        if ($default && Str::stringContains(trim($default), ' ')) {
            $type = 'array';
            $default = explode(' ', $default);
        }

        if (Str::stringEndsWith($name, '*')) {
     ...
     * @return Option
     */
    protected function parseOptionSignature(string $signature) : Option
    {
        $isRequired = false;
        $type = null;

        // If starts with ! it is required
        if (Str::stringStartsWith($signature, '!')) {
            $isRequired = true;
