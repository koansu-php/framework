 ...
 *
 * @see \Koansu\Core\Contracts\Hookable
 * @see \Koansu\Core\Contracts\Subscribable
 *
 */
class ListenerContainer
{
     ...
     */
    public function add(string $event, callable $listener, string $position=self::ON)
    {
     ...
     * @return callable[]
     */
    public function get(string $event, string $position=self::ON) : array
    {
        if (!isset($this->listeners[$position])) {
            return [];
        }
     ...
     * @return callable[]
     */
    public function getByInheritance(string $abstract, $concrete, string $position=self::ON, array &$collected=[]) : array
    {
        if (!isset($this->listeners[$position])) {
            return [];
        }
     ...
     * @return bool
     */
    public function call(string $event, array $args=[], $positions=self::ON) : bool
    {
        $called = false;
     ...
     * @return bool
     */
    public function callByInheritance(string $abstract, $concrete, array $args=[], $positions=self::ON) : bool
    {
        if ($this->muted) {
            return false;
        }
