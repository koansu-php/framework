<?php
/**
 *  * Created by mtils on 05.02.2023 at 09:39.
 **/
...
class JsonPathIterator implements Iterator
{
     ...
     */
    protected function push(RecursiveArrayIterator $iterator, $key=null, bool $raisePosition=true) : void
    {
        $this->stack[] = [
            'iterator'  => $iterator,
            'key'       =>  $key
     ...
     * @return void
     */
    protected function failOnUnsupportedExpression(string $expression) : void
    {
        foreach (['..', '?', '@', '(', ')'] as $criterion) {
            if (strpos($expression, $criterion) !== false) {
                throw new InvalidArgumentException("The json path expression '$criterion' is not supported");
            }
        }
    }
}