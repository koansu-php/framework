<?php
/**
 *  * Created by mtils on 26.10.2022 at 07:31.
 **/

namespace Koansu\Console;
...
class ArgumentVector
{
     ...
     */
    public function __construct(array $argv, bool $removeCommand=true)
    {
        $this->argv = $argv;
     ...
     * @return $this
     */
    public function defineShortOption(string $char, bool $takesValue=false) : ArgumentVector
    {

        if (strlen($char) != 1) {
            throw new LengthException("Short options can have only one char (tried to add:'$char')");
        }
     ...
     * @return array
     */
    public function definedShortOptions() : array
    {
     ...
     * @return bool
     */
    public function isShortOptionDefined(string $option) : bool
    {
     ...
     * @return bool
     */
    public function takesValue(string $option) : bool
    {
     ...
     * @return string
     */
    protected function addLongOption(string $token) : string
    {
        $name = substr($token, 2);
     ...
     * @return string
     */
    protected function addShortOption(string $token) : string
    {
        $name = substr($token, 1);
     ...
     */
    protected function addToOptions(string $name, $value, bool $overwrite=false)
    {
